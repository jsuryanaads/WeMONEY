...
export type TelegramIntentAiResult={intent:"transaction"|"balance"|"expense_summary"|"income_summary"|"budget"|"wallets"|"help"|"unknown";period:"today"|"week"|"month"|"year"|"all"|"unknown";confidence:number};
export async function routeWithOpenRouter(input:{text:string;previousIntent?:string}){if(!OPENROUTER_API_KEY)return null;const prompt=`Classify an Indonesian personal-finance Telegram message. Return JSON only. Message: ${input.text}. Previous intent: ${input.previousIntent||"none"}. Allowed intent: transaction,balance,expense_summary,income_summary,budget,wallets,help,unknown. Allowed period: today,week,month,year,all,unknown. Be conservative. "iya","ya","simpan","oke" should be transaction only when previous intent is transaction. Return intent, period, confidence.`;const controller=new AbortController();const timeout=setTimeout(()=>controller.abort(),7000);try{const response=await fetch("https://openrouter.ai/api/v1/chat/completions",{method:"POST",headers:{"content-type":"application/json",Authorization:`Bearer ${OPENROUTER_API_KEY}`},signal:controller.signal,body:JSON.stringify({model:OPENROUTER_MODEL,temperature:0,response_format:{type:"json_object"},messages:[{role:"system",content:"You are a conservative intent router for WeMONEY. Never invent financial facts."},{role:"user",content:prompt}]})});if(!response.ok)throw new Error(`OpenRouter intent HTTP ${response.status}`);const data=await response.json();const content=data?.choices?.[0]?.message?.content;if(!content)return null;const r=JSON.parse(String(content));const intents=["transaction","balance","expense_summary","income_summary","budget","wallets","help","unknown"];const periods=["today","week","month","year","all","unknown"];return{intent:intents.includes(r?.intent)?r.intent:"unknown",period:periods.includes(r?.period)?r.period:"unknown",confidence:Math.min(Math.max(Number(r?.confidence)||0,0),0.99)} as TelegramIntentAiResult}finally{clearTimeout(timeout)}}
