...
const DEFAULT_EXPENSE_CATEGORIES = [
  ['Makanan', 'Utensils', 'rose'],
  ['Transportasi', 'Car', 'blue'],
  ['Belanja', 'ShoppingBag', 'violet'],
  ['Tagihan', 'ReceiptText', 'amber'],
  ['Hiburan', 'Gamepad2', 'pink'],
  ['Kesehatan', 'HeartPulse', 'emerald'],
  ['Pendidikan', 'GraduationCap', 'indigo'],
  ['Lainnya', 'MoreHorizontal', 'slate'],
]

const DEFAULT_INCOME_CATEGORIES = [
  ['Gaji', 'Wallet', 'emerald'],
  ['Bonus', 'Gift', 'blue'],
  ['Penjualan', 'Store', 'violet'],
  ['Investasi', 'TrendingUp', 'indigo'],
  ['Hadiah', 'Gift', 'pink'],
  ['Lainnya', 'MoreHorizontal', 'slate'],
]
...
export async function ensureUserProfile(user) {
  ...
  const { data: existing, error: categoryReadError } = await supabase
    .from('categories')
    .select('name,type')
    .eq('user_id', user.id)
  ...
  if (missing.length) {
    const { error: categoryInsertError } = await supabase.from('categories').insert(missing)
    if (categoryInsertError) return { error: categoryInsertError }
  }
