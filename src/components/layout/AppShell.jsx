...
export default function AppShell({ title, children }) {
  ...
  return <div className="min-h-screen overflow-x-hidden bg-slate-50 text-slate-800 transition-colors duration-200 lg:pl-64">
    ...
    <header className="sticky top-0 z-30 flex min-h-16 items-center justify-between border-b border-slate-200 bg-white/90 px-3 shadow-sm backdrop-blur-md sm:px-4 lg:px-8">
      <div className="flex min-w-0 items-center"><button className="rounded-lg p-2 hover:bg-slate-100 lg:hidden" onClick={() => setOpen(true)} aria-label="Buka menu"><Menu/></button><h1 className="ml-2 truncate font-bold text-slate-900 lg:ml-0">{title}</h1></div>
      <div className="ml-auto flex items-center gap-1.5 sm:gap-2">
        <button onClick={toggleTheme} aria-label={theme === 'dark' ? 'Gunakan tema terang' : 'Gunakan tema gelap'} title={theme === 'dark' ? 'Tema terang' : 'Tema gelap'} className="grid h-10 w-10 place-items-center rounded-xl text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900">{theme === 'dark' ? <Sun size={19}/> : <Moon size={19}/>}</button>
        <button onClick={() => navigate('/pengaturan')} className="flex min-w-0 items-center gap-2 rounded-xl p-1.5 hover:bg-slate-100"><span className="grid h-9 w-9 shrink-0 place-items-center rounded-full bg-blue-100 text-sm font-bold text-blue-700">{name.slice(0,2).toUpperCase()}</span><span className="hidden max-w-48 truncate text-sm font-semibold sm:block">{name}</span></button>
      </div>
    </header>
