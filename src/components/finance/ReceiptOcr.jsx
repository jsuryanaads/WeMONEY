...
export default function ReceiptOcr({ disabled = false, onVerified, onCancel }) {
  ...
  async function handleFile(file) {
    if (!file) return
    setError(''); setResult(null); setStatus('processing'); setProgress(0); setFileName(file.name)
    try {
      validateReceiptFile(file)
      const parsed = await recognizeReceipt(file, setProgress)
      setResult(parsed); setStatus('review')
    } catch (e) {
      setStatus('idle'); setFileName(''); setError(e.message || 'OCR gagal memproses struk.')
    }
  }
  ...
  return <div className="wm-photo-card rounded-2xl p-4 sm:p-5">
    <div className="flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1"><div className="mb-1 inline-flex rounded-full px-2 py-1 text-[10px] font-extrabold uppercase tracking-wider wm-feature-badge">Fitur Unggulan</div><h3 className="text-lg font-extrabold text-slate-900 dark:text-slate-100">Photo Catat</h3><p className="mt-0.5 text-xs leading-5 text-slate-500 dark:text-slate-400">Foto struk, biarkan We MONEY membaca nominal dan tanggal secara otomatis.</p></div>
    </div>
    <div className="mt-2 flex items-center gap-2 text-[11px] font-semibold text-slate-400"><FileScan size={14}/> Smart Receipt / OCR · diproses sementara di browser</div>

    {status === 'idle' && <label className={`wm-photo-upload mt-4 flex min-h-14 cursor-pointer items-center justify-center gap-2 rounded-2xl px-4 py-4 text-sm font-extrabold ${disabled ? 'pointer-events-none opacity-50' : ''}`}>
      <Upload size={18}/> Ambil / Pilih Foto Struk
      <input disabled={disabled} type="file" accept="image/jpeg,image/png,image/webp" capture="environment" className="hidden" onChange={e => handleFile(e.target.files?.[0] || null)}/>
    </label>}
