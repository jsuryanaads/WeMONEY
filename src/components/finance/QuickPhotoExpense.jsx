...
export default function QuickPhotoExpense({ open, onClose }) {
  ...
  return <div className="fixed inset-0 z-[75] grid items-end justify-center bg-slate-950/60 p-0 backdrop-blur-sm sm:items-center sm:p-4" role="dialog" aria-modal="true" aria-labelledby="quick-photo-title">
    <div className="wm-quick-card max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-[28px] p-4 shadow-2xl sm:rounded-[28px] sm:p-5">
      ...
      {saved ? <div className="grid place-items-center py-12 text-center">
        <div className="grid h-14 w-14 place-items-center rounded-full bg-emerald-100 text-emerald-600 dark:bg-emerald-900/30"><Check size={28} /></div>
        <p className="mt-4 font-extrabold">Pengeluaran tersimpan</p>
        <p className="mt-1 text-sm opacity-60">{money(wallet?.balance ? Number(wallet.balance) : 0)} · {wallet?.name || 'Kas Utama'} · {category?.name || 'Belanja'}</p>
      </div> : <>
        ...
        <ReceiptOcr embedded disabled={saving} autoOpen onVerified={handleVerified} onCancel={onClose} />
