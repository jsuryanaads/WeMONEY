...
export default function TransactionsPageOCR() {
  const { user } = useAuth()
  ...
  async function load() {
    ...
    try {
      ...
      const [result, walletData, categoryData] = await Promise.all([
        getTransactionsPage(user.id, { page, pageSize, ...filters }),
  ...
  async function saveTransfer(e) {
    ...
    try {
      ...
      setError(e.message || 'Gagal menyimpan transfer.')
    } finally {
      setBusy(false)
    }
  }
  ...
  async function remove(id) {
    ...
    try {
      ...
      setError(e.message || 'Gagal menghapus transaksi.')
    }
  }
    ...
    {tab === 'transaction' ? <>
      <form onSubmit={save} className={`${card} grid gap-4 md:grid-cols-2`}>
        <div className="md:col-span-2 flex items-start justify-between gap-3">
          ...
          {editing && <button type="button" onClick={cancelEdit} className="rounded-lg p-2 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800" aria-label="Batal edit"><X size={19}/></button>}
        </div>
      ...
      <div className={`${card} mt-6`}>
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between"><h2 className="font-extrabold text-slate-900 dark:text-slate-100">Riwayat Transaksi</h2><span className="text-xs font-semibold text-slate-400">{total} hasil</span></div>
