...
export default function BudgetsPage() {
  ...
  return <AppShell title="Anggaran">
    <div className="mb-5"><h2 className="text-2xl font-extrabold tracking-tight">Anggaran</h2><p className="mt-1 text-sm text-slate-500">Tetapkan batas pengeluaran dan lihat seberapa dekat kamu dengan batas tersebut.</p></div>
    ...
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.5fr)]">
      <form onSubmit={save} className="wm-panel h-fit">
        ...
        <div className="mt-4 grid gap-3">
          <label className="text-sm font-semibold">Nama<input required className="wm-input mt-1.5" value={form.name} onChange={e=>setForm({...form,name:e.target.value})} placeholder="Contoh: Kebutuhan Bulanan"/></label>
          <label className="text-sm font-semibold">Nominal<input required min="1" type="number" className="wm-input mt-1.5" value={form.amount} onChange={e=>setForm({...form,amount:e.target.value})} placeholder="5000000"/></label>
          <label className="text-sm font-semibold">Kategori<select className="wm-input mt-1.5" value={form.category_id} onChange={e=>setForm({...form,category_id:e.target.value})}><option value="">Semua pengeluaran</option>{categories.map(c=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
          <div className="grid grid-cols-2 gap-3"><label className="text-sm font-semibold">Periode<select className="wm-input mt-1.5" value={form.period} onChange={e=>periodChange(e.target.value)}><option value="weekly">Mingguan</option><option value="monthly">Bulanan</option><option value="yearly">Tahunan</option></select></label><label className="text-sm font-semibold">Mulai<input required type="date" className="wm-input mt-1.5" value={form.start_date} onChange={e=>setForm({...form,start_date:e.target.value})}/></label></div>
          <label className="text-sm font-semibold">Selesai<input type="date" className="wm-input mt-1.5" value={form.end_date} onChange={e=>setForm({...form,end_date:e.target.value})}/></label>
          ...
          <div className="flex gap-2"><button disabled={saving} className="wm-primary flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-extrabold"><Plus size={17}/>{saving?'Menyimpan...':editing?'Simpan Perubahan':'Tambah Anggaran'}</button>{editing&&<button type="button" onClick={reset} className="rounded-xl border px-4 py-3 text-sm font-bold">Batal</button>}</div>
        </div>
      </form>
...
function BudgetCard({ budget, usage, onEdit, onDelete }) {
  ...
  return <article className="wm-panel"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="truncate font-extrabold">{budget.name}</h3>{budget.is_active ? <span className="rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-700">Aktif</span> : <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-bold text-slate-500">Nonaktif</span>}</div><p className="mt-1 text-xs text-slate-500">{budget.category?.name || 'Semua pengeluaran'} · {budget.start_date} — {usage.end}</p></div><div className="flex gap-1"><button onClick={onEdit} className="rounded-lg p-2 opacity-70 hover:bg-black/5" aria-label="Edit anggaran"><Pencil size={16}/></button><button onClick={onDelete} className="rounded-lg p-2 text-rose-600 opacity-70 hover:bg-rose-50" aria-label="Hapus anggaran"><Trash2 size={16}/></button></div></div><div className="mt-5 flex items-end justify-between gap-3"><div><span className="text-xs text-slate-500">Terpakai</span><strong className="mt-1 block text-xl font-extrabold">{money(usage.actual)}</strong></div><div className="text-right"><span className="text-xs text-slate-500">Dari {money(budget.amount)}</span><b className={`mt-1 block ${danger?'text-rose-600':warning?'text-amber-600':'text-emerald-600'}`}>{usage.percent.toFixed(1)}%</b></div></div><div className="wm-progress mt-3 h-2"><i style={{width:`${pct}%`}}/></div><div className="mt-3 flex items-center justify-between gap-3 text-xs"><span className={`flex items-center gap-1 font-bold ${danger?'text-rose-600':warning?'text-amber-600':'text-emerald-600'}`}>{danger?<AlertTriangle size={14}/>:<CheckCircle2 size={14}/>} {usage.exceeded?'Melebihi budget':warning?'Perlu diperhatikan':'Masih aman'}</span><span className="font-semibold text-slate-500">Sisa {money(usage.remaining)}</span></div></article>
}
