...
export default function BudgetsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      if (p) {
        setPlan(p)
        setIncome(String(p.income_amount))
        setObligation(String(p.obligation_amount))
        setAllocations(p.allocations?.length ? p.allocations : defaultAllocations())
      } else {
        setPlan(null)
        setIncome('')
        setObligation('')
      ...
      setError(e.message || 'Gagal memuat anggaran.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const saveLegacy = async e => {
    ...
    try {
      ...
      setError(e.message || 'Gagal menyimpan batas pengeluaran.')
    } finally {
      setSaving(false)
    }
  }
  ...
  const editLegacy = b => {
    setEditing(b.id)
    setLegacyForm({ name: b.name, amount: String(b.amount), period: b.period, start_date: b.start_date, end_date: b.end_date || '', category_id: b.category_id || '', is_active: b.is_active })
  }
  ...
  const removeLegacy = async id => {
    ...
    try {
      ...
      setError(e.message || 'Gagal menghapus batas pengeluaran.')
    }
  }
    ...
    <section className="mb-5 grid gap-4 lg:grid-cols-[minmax(300px,0.85fr)_minmax(0,1.5fr)]">
      <form onSubmit={saveLegacy} className="wm-panel h-fit">
        ...
        <div className="mt-4 grid gap-3">
          ...
          <select className="wm-input" value={legacyForm.category_id} onChange={e => setLegacyForm({ ...legacyForm, category_id: e.target.value })}><option value="">Semua pengeluaran</option>{categories.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}</select>
          <select className="wm-input" value={legacyForm.period} onChange={e => setLegacyForm({ ...legacyForm, period: e.target.value })}><option value="weekly">Mingguan</option><option value="monthly">Bulanan</option><option value="yearly">Tahunan</option></select>
          <div className="grid grid-cols-2 gap-2"><input required className="wm-input" type="date" value={legacyForm.start_date} onChange={e => setLegacyForm({ ...legacyForm, start_date: e.target.value })} /><input className="wm-input" type="date" value={legacyForm.end_date} onChange={e => setLegacyForm({ ...legacyForm, end_date: e.target.value })} /></div>
          <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={legacyForm.is_active} onChange={e => setLegacyForm({ ...legacyForm, is_active: e.target.checked })} />Aktif</label>
          <div className="flex gap-2"><button disabled={saving} className="wm-primary rounded-xl px-4 py-3 text-sm font-extrabold">{editing ? 'Simpan' : 'Tambah'}</button>{editing && <button type="button" onClick={resetLegacy} className="rounded-xl border px-4 py-3 text-sm font-bold">Batal</button>}</div>
        </div>
      </form>
      ...
      <div className="space-y-3">
        ...
        {loading ? <div className="wm-panel">Memuat anggaran...</div> : budgets.map(b => {
          ...
          return <article key={b.id} className="wm-panel">
            <div className="flex items-start justify-between gap-3"><div><h3 className="font-extrabold">{b.name}</h3><p className="text-xs text-slate-500">{b.category?.name || 'Semua pengeluaran'} · {b.period}</p></div><div className="flex gap-1"><button onClick={() => editLegacy(b)} className="grid h-10 w-10 place-items-center rounded-lg"><Pencil size={16} /></button><button onClick={() => removeLegacy(b.id)} className="grid h-10 w-10 place-items-center rounded-lg text-rose-600"><Trash2 size={16} /></button></div></div>
            <div className="mt-4 flex justify-between text-sm"><span>Terpakai <b>{money(u.actual)}</b></span><b className={danger ? 'text-rose-600' : 'text-emerald-600'}>{u.percent.toFixed(1)}%</b></div>
            <div className="wm-progress mt-2 h-2"><i style={{ width: `${Math.min(100, u.percent)}%` }} /></div>
            <div className="mt-2 flex items-center justify-between text-xs font-bold"><span className={danger ? 'text-rose-600' : 'text-emerald-600'}>{danger ? <><AlertTriangle size={13} className="mr-1 inline" />Perlu diperhatikan</> : `Sisa ${money(u.remaining)}`}</span><span>{money(b.amount)}</span></div>
          </article>
        })}
      </div>
    </section>
