  ...
  return <AppShell title="Anggaran">
    ...
    <section className="mb-5">
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        ...
        <div className="col-span-2 border-l-2 border-slate-900 pl-3 sm:col-span-1"><span className="text-[11px] font-bold text-slate-600">Sisa</span><strong className="mt-1 block text-xl font-extrabold">{money(remaining)}</strong></div>
      </div>
    </section>

    <section className="mb-4 border-t border-slate-200 pt-4">
      <div className="flex items-center justify-between gap-2"><div><h3 className="text-sm font-extrabold">Rencana vs Realisasi</h3><p className="mt-0.5 text-[11px] text-slate-500">Atur rencana dan bandingkan dengan transaksi.</p></div><div className="inline-flex rounded-lg border p-0.5"><button type="button" onClick={() => setMode('percent')} className={`rounded-md px-2.5 py-1.5 text-[11px] font-extrabold ${mode === 'percent' ? 'bg-slate-900 text-white' : 'text-slate-500'}`}>%</button><button type="button" onClick={() => setMode('nominal')} className={`rounded-md px-2.5 py-1.5 text-[11px] font-extrabold ${mode === 'nominal' ? 'bg-slate-900 text-white' : 'text-slate-500'}`}>Rp</button></div></div>
      <div className="mt-3 divide-y border-y border-slate-200">
        {allocations.map((a, i) => {
          ...
          return <div key={a.group_key} className="py-3"><div className="grid grid-cols-[1fr_auto] items-center gap-3 sm:grid-cols-[1fr_130px_150px]"><div><b className="block text-sm">{a.name}</b><span className="text-[10px] text-slate-400">Realisasi {money(actual)}</span></div><label className="text-right text-[10px] font-bold text-slate-500">{mode === 'percent' ? '%' : 'Rp'}<input className="wm-input mt-1 w-24 !py-1.5 text-right text-sm" type="number" min="0" max={mode === 'percent' ? '100' : undefined} step="0.1" value={mode === 'percent' ? a.percentage : a.amount} onChange={e => updateAllocation(i, e.target.value)} /></label><div className="col-span-2 text-right sm:col-span-1"><b className="text-xs">Rencana {money(planned)}</b><span className="ml-2 text-[10px] text-slate-400">{used.toFixed(0)}% terpakai</span></div></div><div className="wm-progress mt-2 h-1.5"><i style={{ width: `${Math.min(100, Math.max(0, used))}%` }} /></div></div>
        })}
      </div>
      <div className="mt-2 flex items-center justify-between text-xs font-bold text-slate-600"><span>Total {totalPct.toFixed(1)}% · {money(totalAmount)}</span><span className={complete ? 'text-emerald-700' : 'text-amber-700'}>{complete ? 'Lengkap' : 'Total harus 100%'}</span></div>
