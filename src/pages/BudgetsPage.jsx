...
export default function BudgetsPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      else { setPlan(null); setIncome(''); setObligation(''); setAllocations(defaultAllocations()) }
  ...
  return <AppShell title="Anggaran">
    ...
    <section className="wm-panel mb-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2">{allocations.map((a, i) => { const used = Number(groupUsage[a.group_key] || 0); const pctUsed = a.amount > 0 ? used / a.amount * 100 : 0; const over = used > a.amount && a.amount >= 0; return <div key={a.group_key} className="rounded-2xl border p-4"><div className="flex items-center justify-between gap-3"><div><b>{GROUPS.find(g=>g.key===a.group_key)?.icon} {a.name}</b><p className="mt-0.5 text-xs text-slate-500">Terpakai {money(used)}</p></div><span className="text-xs font-bold text-slate-500">{Number(a.percentage || 0).toFixed(1)}%</span></div><div className="mt-3 grid grid-cols-[1fr_auto] items-end gap-2"><label className="text-xs font-semibold">{mode==='percent'?'Persentase':'Nominal'}<input className="wm-input mt-1 w-full" type="number" min="0" max={mode==='percent'?'100':undefined} step="0.1" value={mode==='percent'?a.percentage:a.amount} onChange={e=>updateAllocation(i,e.target.value)}/></label><strong className="pb-2 text-sm">{money(a.amount)}</strong></div><div className="wm-progress mt-3 h-2"><i style={{width:`${Math.min(100,Math.max(0,pctUsed))}%`}}/></div><div className={`mt-2 flex items-center justify-between text-xs font-bold ${over?'text-rose-600':'text-slate-500'}`}><span>{over?'Melebihi alokasi':'Sisa '+money(Math.max(0,a.amount-used))}</span><span>{pctUsed.toFixed(0)}% terpakai</span></div></div> })}</div>
      <div className={`mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl px-4 py-3 text-sm font-bold ${remaining===0 || Math.abs(totalPct-100)<0.01?'bg-emerald-50 text-emerald-700':'bg-amber-50 text-amber-700'}`}><span>Total alokasi: {totalPct.toFixed(1)}% · {money(totalAmount)}</span><span>{remaining===0?'Tidak ada sisa yang perlu dialokasikan':Math.abs(totalPct-100)<0.01?'Semua sisa sudah dialokasikan':'Atur total menjadi 100%'}</span></div>
