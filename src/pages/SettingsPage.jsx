...
const card = 'rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 transition-colors sm:p-6'
...
const button = 'rounded-xl bg-blue-600 px-4 py-3 text-sm font-bold text-white shadow-sm transition hover:bg-blue-700 disabled:opacity-50'
...
export default function SettingsPage() {
  ...
  return <AppShell title="Pengaturan"><div className="space-y-5 sm:space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-2 lg:gap-6">
      <form onSubmit={save} className={card}><h2 className="font-extrabold text-slate-900">Profil</h2><p className="mt-1 text-sm text-slate-400">Informasi akun WeMoney.</p><label className="mt-5 block text-sm font-semibold">Nama lengkap<input className={input} value={name} onChange={e=>setName(e.target.value)}/></label><label className="mt-4 block text-sm font-semibold">Email<input disabled className={`${input} bg-slate-50`} value={user?.email||''}/></label><button className={`${button} mt-5 w-full sm:w-auto`}>Simpan Profil</button>{saved&&<p className="mt-3 text-sm font-semibold text-emerald-600">Profil tersimpan.</p>}</form>
      <div className={card}><h2 className="font-extrabold text-slate-900">Sesi</h2><p className="mt-1 text-sm text-slate-400">Keluar dari akun di perangkat ini.</p><button onClick={logout} className="mt-5 w-full rounded-xl border border-rose-200 px-4 py-3 text-sm font-bold text-rose-600 transition hover:bg-rose-50 sm:w-auto"><LogOut size={17} className="mr-1 inline"/>Keluar</button></div>
    </div>
    <section className={card}><div className="flex items-center gap-3"><Database className="text-blue-600"/><div><h2 className="font-extrabold text-slate-900">Data & Privasi</h2><p className="text-sm text-slate-400">Kelola, backup, reset, atau hapus data.</p></div></div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2"><div className="rounded-xl border border-slate-200 p-4"><h3 className="font-bold">Export Data</h3><p className="mt-1 text-sm leading-6 text-slate-500">Unduh seluruh transaksi sebagai CSV sebelum melakukan reset.</p><button disabled={busy} onClick={exportData} className={`${button} mt-4 w-full sm:w-auto`}><Download size={17} className="mr-1 inline"/>Export CSV</button></div><div className="rounded-xl border border-rose-200 bg-rose-50/50 p-4"><div className="flex items-center gap-2 text-rose-700"><AlertTriangle size={18}/><h3 className="font-bold">Reset Data Keuangan</h3></div><p className="mt-1 text-sm leading-6 text-rose-700/80">Hapus transaksi & transfer dan kosongkan saldo awal semua dompet. Dompet dan kategori tetap.</p><label className="mt-3 block text-xs font-bold text-rose-700">Ketik RESET<input className={`${input} border-rose-200`} value={resetText} onChange={e=>setResetText(e.target.value.toUpperCase())} placeholder="RESET"/></label><button disabled={busy || resetText!=='RESET'} onClick={resetData} className="mt-3 w-full rounded-xl bg-rose-600 px-4 py-3 text-sm font-bold text-white transition hover:bg-rose-700 disabled:opacity-50 sm:w-auto"><Trash2 size={17} className="mr-1 inline"/>Reset Data</button></div></div>
    ...
    <section className={card}><div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-extrabold text-slate-900">Bantuan</h2><p className="mt-1 text-sm text-slate-400">Pelajari alur pencatatan, transfer, saldo, dan keamanan data.</p></div><a href={`${import.meta.env.BASE_URL}panduan`} className="inline-flex w-full items-center justify-center rounded-xl border border-slate-200 px-4 py-3 text-sm font-bold text-blue-600 transition hover:bg-blue-50 sm:w-auto"><BookOpen size={17} className="mr-1"/>Buka Panduan</a></div></section>
    <p className="text-center text-xs text-slate-400">WeMoney V1 | © {new Date().getFullYear()} Created Jsuryana</p>
  </div></AppShell>
}

function Metric({label,value}) { return <div className="rounded-xl bg-slate-50 p-4 transition-colors"><p className="text-xs text-slate-500">{label}</p><p className="mt-1 text-xl font-extrabold text-slate-900">{Number(value||0).toLocaleString('id-ID')}</p></div> }
