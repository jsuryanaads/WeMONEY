...
export default function SettingsPage() {
  ...
  return <AppShell title="Pengaturan">
    <div className="mx-auto max-w-3xl space-y-4 sm:space-y-5">
      ...
      <SettingItem icon={Smartphone} title="Perangkat Saya" description={`${devices.length} perangkat terhubung ke akun`} onClick={() => setModal('devices')} />
      <SettingItem icon={User} title="Profil" description="Nama dan email akun" onClick={() => setModal('profile')} />
      <SettingItem icon={Bot} title="Telegram" description="Integrasi privat untuk mencatat lewat Bot Telegram" onClick={() => setModal('telegram')} />
    ...
    {modal === 'telegram' && <Modal title="Telegram" icon={Bot} onClose={closeModal}>
      <div className="space-y-4">
        <div className="rounded-2xl bg-slate-50 p-4">
          <p className="font-extrabold text-slate-900">Bot Telegram We MONEY</p>
          <p className="mt-1 text-sm leading-6 text-slate-600">Hubungkan akun secara pribadi untuk membuat catatan transaksi dari Telegram. Kode koneksi bersifat sekali pakai dan memiliki masa berlaku terbatas.</p>
        </div>
        <button type="button" onClick={() => { closeModal(); window.history.pushState({}, '', '/telegram'); window.dispatchEvent(new PopStateEvent('popstate')) }} className="flex w-full items-center gap-3 rounded-xl border border-slate-200 px-4 py-3 text-left text-sm font-bold text-slate-800 transition hover:bg-slate-50"><span className="grid h-9 w-9 place-items-center rounded-lg bg-slate-100 text-slate-600"><Bot size={18}/></span><span className="flex-1">Buka Pengaturan Bot Telegram</span><ChevronRight size={18} className="text-slate-400" /></button>
      </div>
    </Modal>}
