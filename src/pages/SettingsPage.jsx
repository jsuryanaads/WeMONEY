...
import { AlertTriangle, Download, LogOut, Database, BookOpen, Trash2 } from 'lucide-react'
import AppShell from '../components/layout/AppShell'
import { useAuth } from '../hooks/useAuth'
import { supabase } from '../lib/supabase'
import { signOut } from '../services/authService'
import { exportTransactionsCsv, getDataStats, resetFinancialData } from '../services/dataService'
...
export default function SettingsPage() {
  ...
  return <AppShell title="Pengaturan"><div className="space-y-6">
    ...
    <section className={card}><div className="flex items-center gap-3"><Database className="text-blue-600"/><div><h2 className="font-extrabold">Data & Privasi</h2><p className="text-sm text-slate-400">Kelola, backup, dan reset data keuangan.</p></div></div>
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-rose-200 bg-rose-50/50 p-4"><div className="flex items-center gap-2 text-rose-700"><AlertTriangle size={18}/><h3 className="font-bold">Reset Data Keuangan</h3></div><p className="mt-1 text-sm leading-6 text-rose-700/80">Menghapus transaksi & transfer dan mengembalikan saldo awal semua dompet ke Rp 0. Dompet dan kategori tetap dipertahankan.</p><label className="mt-3 block text-xs font-bold text-rose-700">Ketik RESET untuk melanjutkan<input className={`${input} border-rose-200`} value={resetText} onChange={e=>setResetText(e.target.value.toUpperCase())} placeholder="RESET"/></label><button disabled={busy || resetText!=='RESET'} onClick={resetData} className="mt-3 rounded-xl bg-rose-600 px-4 py-3 text-sm font-bold text-white hover:bg-rose-700 disabled:opacity-50"><Trash2 size={17} className="mr-1 inline"/>Reset Data</button></div>
      </div>
    </section>
